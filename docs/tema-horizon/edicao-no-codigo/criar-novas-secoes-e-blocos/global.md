...
Todos os temas precisam definir configurações que são aplicadas globalmente, independente da seção ou página. Para isso criamos o arquivo `global.json` no novo editor. Disponível na pasta `configs/editor`, ele permite definir configurações que afetam todo o tema.&#x20;
...
Abaixo um exemplo desse arquivo com algumas das configurações disponíveis no Tema Horizon
